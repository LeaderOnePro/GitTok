...
function createGitTokItem(repo) {
    ...
    // 0. Blurred Background
    const blurredBg = document.createElement('div');
    blurredBg.classList.add('blurred-background');
    if (repo.imageUrl) {
        blurredBg.style.backgroundImage = `url(${repo.imageUrl})`;
    }
    item.appendChild(blurredBg);

    // 1. Content Image
    const contentImage = document.createElement('div');
    contentImage.classList.add('content-image');
    if (repo.imageUrl) {
        contentImage.style.backgroundImage = `url(${repo.imageUrl})`;
    }
    contentImage.addEventListener('click', () => window.open(repo.url, '_blank'));
    item.appendChild(contentImage);
    ...
    infoLeft.innerHTML = `
        <h2>${repo.name}</h2>
        <p>⭐ ${repo.stars} | ${repo.language || 'N/A'}</p>
    `;
    item.appendChild(infoLeft);

    const infoRight = document.createElement('div');
    infoRight.classList.add('info-right');
    infoRight.innerHTML = `
        <p class="repo-description">${repo.description || 'No description available.'}</p>
        ${repo.aiSummary ? `<div class="ai-summary"><strong>AI 总结:</strong> <p>${repo.aiSummary}</p></div>` : ''}
    `;
    ...
    const shareButton = document.createElement('button');
    shareButton.classList.add('share-button');
    shareButton.innerHTML = '🔗'; // Link emoji or use an icon font
    shareButton.title = '复制仓库链接';
    ...
    deepWikiButton.classList.add('deepwiki-btn'); // Add class for styling
    deepWikiButton.innerHTML = 'DW'; // Or use an icon/full text
    deepWikiButton.title = '在 DeepWiki 中打开'; // Update title
    deepWikiButton.addEventListener('click', (e) => {
        e.stopPropagation(); // Prevent triggering image click
        // Construct DeepWiki URL by replacing github.com with deepwiki.com
        let deepWikiUrl = repo.url; // Start with the original GitHub URL
        if (deepWikiUrl && deepWikiUrl.includes('github.com')) {
            ...
            // Optional: Fallback to search if direct replacement fails
            const repoNameForSearch = encodeURIComponent(repo.name.split('/').pop());
            deepWikiUrl = `https://deepwiki.com/search?q=${repoNameForSearch}`; // Use .net for search as before
        }
        window.open(deepWikiUrl, '_blank');
    });
    // Append to infoRight or another suitable container
...
async function fetchAndDisplayTrending(since = 'daily') {
    const feed = document.querySelector('.gittok-feed');
    // Clear previous items except the initial loading message if it's still there
    const initialMessage = feed.querySelector('.gittok-item h1');
    if (initialMessage && initialMessage.textContent.includes('欢迎')) {
        feed.innerHTML = ''; // Clear only if it's the welcome message
    } else if (!initialMessage) {
        // If not the welcome message, clear everything for refresh
        feed.innerHTML = '';
    }

    // Optional: Show a loading indicator while fetching
    const loadingIndicator = document.createElement('div');
    loadingIndicator.classList.add('gittok-item'); // Reuse styling if appropriate
    loadingIndicator.innerHTML = '<p>正在加载...</p>';
    if (feed.innerHTML === '') { // Add loading only if feed is empty
        feed.appendChild(loadingIndicator);
    }

    try {
        // Fetch trending data from your API endpoint
        const response = await fetch(`/api/trending?since=${since}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const repos = await response.json();

        // Clear loading indicator if it was added
        if (feed.contains(loadingIndicator)) {
            feed.removeChild(loadingIndicator);
        }

        // Populate feed
        repos.forEach(repo => {
            const item = createGitTokItem(repo);
            feed.appendChild(item);
        });

        // If it was the initial load, remove the placeholder if it still exists
        // (Should have been cleared earlier, but as a safeguard)
        const placeholder = feed.querySelector('.gittok-item h1');
        if (placeholder && placeholder.textContent.includes('欢迎')) {
             feed.removeChild(placeholder.parentNode);
        }
